...
using System.Data;
class Complex<T> : IComparable<Complex<T>>, IFormattable where T : struct, IComparable<T>
{
    T realPart;
    T imaginaryPart;
    ...
    {
        string realString = Convert.ToString(realPart, formatProvider);
        string imaginaryString = Convert.ToString(imaginaryPart, formatProvider);
        return $"{realString} + {imaginaryString}i";
    }
    ...
    public int CompareTo(Complex<T>? other)
    {
        ...
        int realComparison = realPart.CompareTo(other.realPart);
        if (realComparison != 0)
        {
            return realComparison;
        }

        return imaginaryPart.CompareTo(other.imaginaryPart);
    }
}
public class Matrix<T> : IComparable<Matrix<T>>, IFormattable where T : IComparable<T>
{
    private T[,] elements;
    private int rows;
    private int cols;
...
public class SquareMatrix<T> : Matrix<T> where T : IComparable<T>
{
...
class Program
{
    static void Main(string[] args)
    {
        ...
        SquareMatrix<Complex<int>> m6 = new SquareMatrix<Complex<int>>(2);
        m6.SetElement(0, 0, new Complex<int>(1, 2));
        m6.SetElement(0, 1, new Complex<int>(0, 0));
        m6.SetElement(1, 0, new Complex<int>(0, 0));
        m6.SetElement(1, 1, new Complex<int>(3, 4));
        m6.print();
